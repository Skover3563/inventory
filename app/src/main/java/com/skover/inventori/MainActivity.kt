...
import com.dmitrysukhov.inventory.TEST_SCREEN
import com.skover.inventori.ui.theme.LIST_SCREEN
import com.skover.inventori.ui.theme.Listscreen
...
fun MyApp() {
    ...
    { padding ->
        ...
        NavHost(
            ...
            composable(SCREEN_FOREST) { ScreenForest(navController) }
            composable(SCREEN_CAVE) { ScreenCave(navController) }
            composable(SCREEN_CAVE2) { ScreenCave2(navController) }
            composable(SCREEN_STRANIK) { ScreenStranik(navController) }
            composable(SCREEN_WICKED) { ScreenWicked(navController) }
            composable(SCREEN_STRANIK2) { ScreenStranik2(navController) }
            composable(SCREEN_CLIND) { ScreenClind(navController) }
            composable(SCREEN_EXIT) { ScreenExit(navController) }
...
fun MainScreen(navController: NavHostController) {
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Button(onClick = { navController.navigate(SCREEN_FOREST) }) {
            Text(text = "Игра")
        }
